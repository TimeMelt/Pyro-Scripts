...
func change_scene_to_file(path: String, duration: float = DEFAULT_DURATION, delay: float = DEFAULT_DELAY):
	if duration <= 0.0:
		push_error("TRANSIT ERROR: change_scene_to_file duration must be > 0. Defaulting to %s" % DEFAULT_DURATION)
		duration = DEFAULT_DURATION

	if delay < 0.0:
		push_error("TRANSIT ERROR: change_scene_to_file delay must be >= 0. Defaulting to %s" % DEFAULT_DELAY)
		delay = DEFAULT_DELAY
	...
	_animator.set_speed_scale(1.0 / duration)
	_animator.play("fade")
